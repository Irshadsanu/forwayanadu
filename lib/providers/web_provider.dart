...
class WebProvider extends ChangeNotifier{
      ...
      if(event.docs.isNotEmpty){
        ...
        for (var element in event.docs) {
          ...
          if( element.data().containsKey("Receipt Status")){
            // receiptStatus="Viewed";
            receiptStatus=element.get('Receipt Status').toString();
          }
  ...
  void uploadWard(List map, BuildContext context) async{

    for(var data in map){
      print("aaaaaaaaaa"+data['Value.wardname'].toString());
      print("bbbbbb"+data .toString());
      HashMap<String, Object> excelData = HashMap();
      ...
      // excelData['Target']=data['Value.target'];

      await mRoot.child('NewWards').child(data['Value.wardnumber'].toString()+"_"+data['Value.panchayath']).set(excelData);
      // +"_"+data['Value.panchayath']
      HashMap<String, Object> fireStore = HashMap();
      HashMap<String, Object> fireStore1 = HashMap();
      ...
      //PANCHAYATH
      fireStore1['AMOUNT']=0;
      fireStore1['ASSEMBLY']=data['Value.assembly'].toString();
      fireStore1['DISTRICT']=data['Value.district'].toString();
      fireStore1['PANCHAYATH']=data['Value.panchayath'].toString();
      fireStore1['TRANSACTION_COUNT']=0;

      //ASSEMBLY
      fireStore2['AMOUNT']=0;
      fireStore2['ASSEMBLY']=data['Value.assembly'].toString();
      ...
      print("${'${data['Value.district'].toString()}'
          '_${data['Value.panchayath'].toString()}_${data['Value.wardname'].toString()}'} " +"993929392");
      //
      await db.collection('WardTotalAmount').doc('${data['Value.district'].toString()}'
          '_${data['Value.panchayath'].toString()}_${data['Value.wardname'].toString()}').set(fireStore);

      // await db.collection("PANCHAYATH_TOTAL").doc('${data['Value.district'].toString()}'
      //     '_${data['Value.assembly'].toString()}_${data['Value.panchayath'].toString()}').set(fireStore1);
      //
      //  await db.collection("ASSEMBLY_TOTAL").doc('${data['Value.district'].toString()}'
      //      '_${data['Value.assembly'].toString()}').set(fireStore2);
      //
      //  await db.collection("DISTRICTS_TOTAL").doc(data['Value.district'].toString()).set(fireStore3);
  ...
  void hideWard(List map, BuildContext context) async{
    ...
    for(var data in map) {
      ...
      await mRoot.child('hidewards').child(data['Value.wardnumber'].toString()).set(excelData);
      await db.collection('WardTotalAmount').doc('${data['Value.district'].toString()}''_${data['Value.panchayath'].toString()}_${data['Value.wardname'].toString()}').delete();
    }
  }
  ...
  Future<void> makeReceipt(String subCommittee) async {
    ...
    timeString=DateFormat('d/MM/yyy').format(now);
    ...
    //   print(now.toString()+"ttyyuu");
    //   print(timeString+"ttyyuu");
    // }
    ...
    recieptData['SubCommitty']=subCommittee;
    String district = '';
    String assembly = '';
    String panchayath = '';
    String wardName = '';
    String wardNumber= '';
    if (wardModel != null) {
      district = wardModel!.district;
      assembly = wardModel!.assembly;
      panchayath = wardModel!.panchayath;
      wardName = wardModel!.wardName;
      wardNumber = wardModel!.wardNumber;
    } else {
      district = 'General';
      assembly = 'General';
      panchayath = 'General';
    ...
    print( wardModel!.panchayath.toString()+"panchayath");
    print( wardModel!.wardName.toString()+"wardName");
    print( wardModel!.wardNumber.toString()+"wardNumber");
    print( wardModel!.assembly.toString()+"assembly");
    print( wardModel!.district.toString()+"district");

    recieptData["district"]=district;
    recieptData["assembly"]=assembly;
    recieptData["panchayath"]=panchayath;
    recieptData["wardName"]=wardName;
    recieptData["wardNumber"]=wardNumber;
    recieptData["Ward"]=wardName;
    recieptData["UpiID"] = "";
    recieptData["RefNo"] = "";
  ...
  void clearMakeReceipt(){
    webName.clear();
    webPhone.clear();
    webAmount.clear();
    webTransactionID.clear();
 ...
 loopPayments(int start,int end){
  ...
  Future<void> testWeb() async {
    ...
    int i=0;
    fireStore["TEST"] =0;
    await db.collection('TestWarddddd').doc("test").set(fireStore,SetOptions(merge: true)).then((value) {
      i++;
      print(i.toString()+"  yyeyeeeyey");
    });
  }
  ...
  Future<void> loopTotalDistrict() async {
    ...
    jsonMap.forEach((key, value) async {
      ...
      await db.collection('DISTRICTS_TOTAL').doc(value['district'].toString()).set(fireStore).then((value) {
        i++;
        print(i.toString()+"  yyeyeeeyey");
      });
...
Future<void> looptime() async {
  ...
  // Fetch all documents in the Payments collection
  ...
  for (QueryDocumentSnapshot doc in snapshot.docs) {
    ...
    if (data.containsKey('Time')) {
      dynamic timeValue = data['Time'];

      // Check if the Time field is a string
      if (timeValue is String) {
        try {
          // Attempt to convert the string to a number
          num convertedTime = num.parse(timeValue);

          // Log conversion details
          print('Document ID: ${doc.id}');
          print('Original Time value (String): $timeValue');
          print('Converted Time value (Number): $convertedTime');

          // Update the document with the converted Time value
          // await payments.doc(doc.id).update({'Time': convertedTime});
          await payments.doc(doc.id).set({'Time': convertedTime}, SetOptions(merge: true));

          // Log successful update
          print('Document ID: ${doc.id} updated successfully.\n');
        } catch (e) {
          // Log error in case of conversion failure
          print('Error converting Time value for Document ID: ${doc.id}');
          print('Original Time value: $timeValue');
          print('Error: $e\n');
        }
      } else if (timeValue is num) {
        // Log if Time value is already a number
        print('Document ID: ${doc.id}');
        print('Time value is already a number: $timeValue\n');
      } else {
        // Log if Time value is neither a string nor a number
        print('Document ID: ${doc.id}');
        print('Time value is neither a string nor a number: $timeValue\n');
      }
    } else {
      // Log if the Time field does not exist
      print('Document ID: ${doc.id}');
      print('Time field does not exist.\n');
    }
  }
  ...
  void uploanamewhiteguard(List map, BuildContext context) async{

    for(var data in map){


      db.collection('USER POOLS').doc(data['id'].toString()).set({"NAME":data['Value.name'].toString()},SetOptions(merge: true));
